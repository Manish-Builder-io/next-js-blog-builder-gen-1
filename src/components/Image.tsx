...
export const getSizes = (sizes, block) => {
  let useSizes = '';
...
class ImageComponent extends React.Component {
  ...
  render() {
    ...
    if (typeof image == 'string') {
      ...
      if (image.derived) {
        srcset = `
          ${image?.derived[0]?.secure_url.replace(` `, `\ `)} 1920w,
          ${image?.derived[1]?.secure_url.replace(` `, `\ `)} 1600w,
          ${image?.derived[2]?.secure_url.replace(` `, `\ `)} 1366w,
          ${image?.derived[3]?.secure_url.replace(` `, `\ `)} 1000w,
          ${image?.derived[4]?.secure_url.replace(` `, `\ `)} 768w,
          ${image?.derived[5]?.secure_url.replace(` `, `\ `)} 640w,
        `;
      }
    } else if (srcset && image && image.includes('builder.io/api/v1/image')) {
          ...
          const imageContents = (!lazy || this.state.load || amp) && (
            <Tag
              {...(amp
                ...
                : null)}
              alt={this.props.altText ? this.props.altText : image?.context?.custom.alt}
              key={
                Builder.isEditing
                  ? (typeof this.image.secure_url === 'string' && this.image.secure_url.split('?')[0]) || undefined
                  : undefined
              }
